      ...
      >
        <div className="relative mb-2 flex h-40 items-center justify-center overflow-hidden rounded-lg bg-secondary/50 p-3">
          <img src={product.image} alt={product.title}
            className="h-full max-h-32 w-auto object-contain transition-transform duration-300 group-hover:scale-105" loading="lazy" />
          {discount > 0 && (
        ...
        <p className="mb-0.5 text-[10px] font-medium uppercase tracking-wider text-muted-foreground">{product.category}</p>
        <h3 className="mb-1.5 line-clamp-2 text-xs font-semibold leading-tight text-card-foreground flex-1">{product.title}</h3>
        ...
        <div className="flex items-center gap-1.5">
          <motion.button whileTap={{ scale: 0.8 }}
            onClick={e => { e.stopPropagation(); requireAuth(() => toggleLike(product.id)); }}
          ...
          <motion.button whileTap={{ scale: 0.8 }}
            onClick={e => { e.stopPropagation(); requireAuth(() => addToCart(product.id)); }}
          ...
          <motion.button whileTap={{ scale: 0.95 }} onClick={openInBrowser}
